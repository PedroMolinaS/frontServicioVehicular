...
export const getAllServices = async () => {
    try {
        const response = await fetch(`${URL_BACKEND}/services`)
...
export const getAServicesFiltered = async (category) => {
    try {
        const response = await fetch(`${URL_BACKEND}/services/${category}`)
        const json = await response.json()
        return json
    } catch (error) {
        console.error(error)
...
export const putService = async (token,id, data) => {
    try {
        const response = await fetch(`${URL_BACKEND}/services/${id}`,{
            method: 'PUT',
            headers: { 
                'Content-Type': 'application/json',
                'Authorization': token
